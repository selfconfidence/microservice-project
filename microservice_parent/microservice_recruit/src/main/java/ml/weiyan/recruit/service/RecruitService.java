package ml.weiyan.recruit.service;

import ml.weiyan.recruit.dao.RecruitDao;
...
public class RecruitService {
	...
	public List<Recruit> findAll() {
		Specification<Recruit> specification = createSpecification(new HashMap());
		...
		return new Specification<Recruit>() {
			...
			public Predicate toPredicate(Root<Recruit> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                ...
                if (searchMap.get("state")!=null && !"".equals(searchMap.get("state"))) {
					...
					//必须加的判断,状态不可为0,0是已经关闭的职位,业务莫须有显示出来
					predicateList.add(cb.notEqual(root.get("state").as(String.class), "0"));
    ...
    public List<Recruit> searchRecommend(String s) {
     return recruitDao.findByStateOrderByCreatetimeDesc(s);
    }

	public List<Recruit> searchNewlist(String s) {
		return recruitDao.findByStateOrderByCreatetimeDesc(s);
	}
}
